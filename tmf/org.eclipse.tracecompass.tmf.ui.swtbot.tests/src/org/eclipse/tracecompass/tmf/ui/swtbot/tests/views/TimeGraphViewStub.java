...
public class TimeGraphViewStub extends AbstractTimeGraphView {
    ...
    protected @NonNull List<IMarkerEvent> getViewMarkerList(Iterable<@NonNull TimeGraphEntry> entries, long startTime, long endTime, long resolution, @NonNull IProgressMonitor monitor) {
        ...
        return Arrays.<IMarkerEvent> asList(
                ...
                (IMarkerEvent) new MarkerEvent(fRow2, 20 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", true, 0),
                (IMarkerEvent) new MarkerEvent(fRow2, 26 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", false, 1),
                (IMarkerEvent) new MarkerEvent(fRow2, 32 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", true, 2),
                (IMarkerEvent) new MarkerEvent(fRow2, 38 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", false, 3),
                (IMarkerEvent) new MarkerEvent(fRow2, 44 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", true, 4),
                (IMarkerEvent) new MarkerEvent(fRow2, 50 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", false, 5),
                (IMarkerEvent) new MarkerEvent(fRow2, 56 + start, 0, MARKER_GROUP, new RGBA(160, 170, 200, 80), "", true, 6));
    }
