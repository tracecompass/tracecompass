...
/**
 * Zoom-out handler for TMF views when zooming based on time selection window.
 *
 * @author Bernd Hufmann
 */
public class TmfViewSelectionZoomOutHandler extends TmfViewBaseHandler {

    @Override
    public void execute(TmfView view) {
        ITmfTimeZoomProvider zoomer = view.getAdapter(ITmfTimeZoomProvider.class);
        if (zoomer != null) {
            zoomer.zoom(false, false);
        }
    }
}
